...
COOKIE_FILE = "Shouko.dev/cookies.txt"  # New cookie file path

version = "2.2.6 | Enhanced Cookie Manager by Shouko.dev"
...
class FileManager:
    ...
    # New cookie management methods
    @staticmethod
    def save_cookies(cookies):
        """Save multiple cookies to file"""
        try:
            with open(FileManager.COOKIE_FILE, "w") as file:
                for cookie in cookies:
                    file.write(f"{cookie}\n")
            print("\033[1;32m[ Shouko.dev ] - Cookies saved successfully.\033[0m")
            return True
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error saving cookies: {e}\033[0m")
            Utilities.log_error(f"Error saving cookies: {e}")
            return False
    ...
    def load_cookies():
        ...
        cookies = []
        if os.path.exists(FileManager.COOKIE_FILE):
            with open(FileManager.COOKIE_FILE, "r") as file:
                for line in file:
                    cookie = line.strip()
                    if cookie:
    ...
    def add_cookie(cookie):
        """Add a single cookie to the file"""
        try:
            with open(FileManager.COOKIE_FILE, "a") as file:
                file.write(f"{cookie}\n")
            print("\033[1;32m[ Shouko.dev ] - Cookie added successfully.\033[0m")
            return True
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error adding cookie: {e}\033[0m")
            Utilities.log_error(f"Error adding cookie: {e}")
            return False

    @staticmethod
    def clear_cookies():
        """Clear all cookies from file"""
        try:
            with open(FileManager.COOKIE_FILE, "w") as file:
                file.write("")
            print("\033[1;32m[ Shouko.dev ] - Cookies cleared successfully.\033[0m")
            return True
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error clearing cookies: {e}\033[0m")
            Utilities.log_error(f"Error clearing cookies: {e}")
            return False
...
class RobloxManager:
    @staticmethod
    def get_cookie():
        """Get a cookie from the cookie file (legacy method)"""
        try:
    ...
    # New cookie management methods
    @staticmethod
    def login_with_cookie(cookie_value, package_name=None):
        """Login to Roblox using a cookie and optionally launch app"""
        try:
            # Verify the cookie first
            user_id = RobloxManager.verify_cookie(cookie_value)
            if not user_id:
                print("\033[1;31m[ Shouko.dev ] - Invalid cookie, login failed.\033[0m")
                return False
            
            print(f"\033[1;32m[ Shouko.dev ] - Login successful for user ID: {user_id}\033[0m")
            
            # If package name is provided, inject cookie and launch app
            if package_name:
                RobloxManager.inject_cookie_to_package(cookie_value, package_name)
                RobloxManager.launch_roblox(package_name, "roblox://placeID=0")
                
            return True
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error during login: {e}\033[0m")
            Utilities.log_error(f"Login error: {e}")
            return False

    @staticmethod
    def inject_cookie_to_package(cookie_value, package_name):
        """Inject cookie to a specific package"""
        try:
            ...
            if not downloaded_db_path or not downloaded_appstorage_path:
                ...
                Utilities.log_error("Failed to download necessary files for cookie injection.")
                return False
            ...
            # Replace cookie value in database
            RobloxManager.replace_cookie_value_in_db(destination_db_path, cookie_value)

            return True
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error injecting cookie: {e}\033[0m")
            Utilities.log_error(f"Cookie injection error: {e}")
            return False

    @staticmethod
    def logout_all_packages():
        """Logout from all packages by clearing cookies"""
        try:
            packages = RobloxManager.get_roblox_packages()
            if not packages:
            ...
            for package_name in packages:
                try:
                    # Clear cookies database
                    cookies_path = f"/data/data/{package_name}/app_webview/Default/Cookies"
                    if os.path.exists(cookies_path):
                        os.remove(cookies_path)
                        print(f"\033[1;32m[ Shouko.dev ] - Cleared cookies for {package_name}\033[0m")
                    
                    # Clear appStorage
                    appstorage_path = f"/data/data/{package_name}/files/appData/LocalStorage/appStorage.json"
                    if os.path.exists(appstorage_path):
                        os.remove(appstorage_path)
                        print(f"\033[1;32m[ Shouko.dev ] - Cleared appStorage for {package_name}\033[0m")
                    
                    # Kill the process
                    RobloxManager.kill_roblox_process(package_name)
                except Exception as e:
                    print(f"\033[1;31m[ Shouko.dev ] - Error logging out {package_name}: {e}\033[0m")
                    Utilities.log_error(f"Logout error for {package_name}: {e}")
                    continue
            
            print("\033[1;32m[ Shouko.dev ] - Successfully logged out from all packages.\033[0m")
            return True
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error during mass logout: {e}\033[0m")
            Utilities.log_error(f"Mass logout error: {e}")
            return False

    @staticmethod
    def generate_cookie(username, password):
        """Generate a new cookie using username and password"""
        try:
            # First get the X-CSRF-TOKEN
            session = requests.Session()
            response = session.post("https://auth.roblox.com/v2/login", json={})
            csrf_token = response.headers.get("x-csrf-token")
            if not csrf_token:
                print("\033[1;31m[ Shouko.dev ] - Failed to get CSRF token.\033[0m")
                return None
            ...
            if response.status_code != 200:
                print(f"\033[1;31m[ Shouko.dev ] - Login failed: {response.status_code} - {response.text}\033[0m")
                return None
            
            # Extract the cookie
            cookie = None
            for cookie_dict in session.cookies:
                if cookie_dict.name == ".ROBLOSECURITY":
                    cookie = cookie_dict.value
                    break
            
            if not cookie:
                print("\033[1;31m[ Shouko.dev ] - Failed to extract cookie from response.\033[0m")
                return None
            
            print("\033[1;32m[ Shouko.dev ] - Successfully generated new cookie.\033[0m")
            return cookie
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error generating cookie: {e}\033[0m")
            Utilities.log_error(f"Cookie generation error: {e}")
            return None

    @staticmethod
    def bypass_cookie_verification(cookie_value):
        """Bypass cookie verification (for educational purposes only)"""
        try:
            # This is a placeholder for any bypass logic
            # In a real implementation, this might involve modifying headers or other techniques
            print("\033[1;33m[ Shouko.dev ] - Bypassing cookie verification...\033[0m")
            
            # For demonstration, we'll just return the same cookie
            # In a real bypass, you might modify the cookie or add additional headers
            return cookie_value
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error bypassing verification: {e}\033[0m")
            Utilities.log_error(f"Bypass error: {e}")
            return None
...
class ExecutorManager:
    ...
    def write_lua_script(detected_executors):
        ...
        try:
            with open("Shouko.dev/checkonline.lua", "r") as f:
                lua_script_content = f.read()
        except Exception as e:
            console.print(f"[bold red][ Shouko.dev ] - Error reading checkonline.lua: {e}[/bold red]")
            return

        # Ghi script vào các executor
        for executor_name in detected_executors:
            ...
            possible_paths = [
                ...
                os.path.join(base_path, "autoexec"),
                os.path.join(base_path, "Workspace"),
                os.path.join(base_path, "workspace")
            ]
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    try:
                        script_path = os.path.join(path, "checkonline.lua")
                        with open(script_path, "w") as f:
                            f.write(lua_script_content)
                        console.print(f"[bold green][ Shouko.dev ] - Script written to: {script_path}[/bold green]")
                        break
                    except Exception as e:
                        console.print(f"[bold red][ Shouko.dev ] - Error writing to {path}: {e}[/bold red]")
    ...
    def check_executor_and_rejoin(package_name, server_link, next_package_event):
        ...
        if detected_executors:
            ...
            while time.time() - start_time < max_wait_time:
                if ExecutorManager.check_executor_status(package_name, continuous=False):
                    globals()["package_statuses"][package_name]["Status"] = "\033[1;32mExecutor loaded successfully\033[0m"
                    UIManager.update_status_table()
            ...
            if not executor_loaded:
                print(f"\033[1;31m[ Shouko.dev ] - Executor didn't load for {package_name}. Rejoining...\033[0m")
                ...
                # Check again
                if ExecutorManager.check_executor_status(package_name, continuous=False):
                    globals()["package_statuses"][package_name]["Status"] = "\033[1;32mExecutor loaded after rejoin\033[0m"
                    UIManager.update_status_table()
                else:
                    print(f"\033[1;31m[ Shouko.dev ] - Still no executor after rejoin for {package_name}\033[0m")
                    globals()["package_statuses"][package_name]["Status"] = "\033[1;31mStill no executor after rejoin\033[0m"
                    UIManager.update_status_table()
            
            # Signal to move to next package regardless of success
    ...
    def reset_executor_file(package_name):
        try:
            for workspace in globals()["workspace_paths"]:
                ...
                if os.path.exists(file_path):
                    os.remove(file_path)
                    print(f"\033[1;32m[ Shouko.dev ] - Reset executor file for {package_name}\033[0m")
        except Exception as e:
            print(f"\033[1;31m[ Shouko.dev ] - Error resetting executor file for {package_name}: {e}\033[0m")
            Utilities.log_error(f"Error resetting executor file for {package_name}: {e}")
...
def main():
    ...
    while True:
        ...
        menu_options = [
            ...
            "Cookie Management Tools"  # New option
        ]
        ...
        if setup_type == "1":
            ...
            # New Cookie Management Menu
            try:
                while True:
                    Utilities.clear_screen()
                    UIManager.print_header(version)
                    
                    print("\033[1;35m[ Shouko.dev ] - Cookie Management Tools\033[0m")
                    print("\033[1;36m1. Login with Cookie\033[0m")
                    print("\033[1;36m2. Generate New Cookie\033[0m")
                    print("\033[1;36m3. Bypass Cookie Verification\033[0m")
                    print("\033[1;36m4. Mass Logout All Packages\033[0m")
                    print("\033[1;36m5. View Saved Cookies\033[0m")
                    print("\033[1;36m6. Add Cookie to Storage\033[0m")
                    print("\033[1;36m7. Clear All Cookies\033[0m")
                    print("\033[1;31m8. Back to Main Menu\033[0m")
                    
                    choice = input("\033[1;93m[ Shouko.dev ] - Select option: \033[0m").strip()
                    
                    if choice == "1":
                        # Login with cookie
                        package_name = input("\033[1;93m[ Shouko.dev ] - Enter package name (leave empty for all): \033[0m").strip()
                        cookie = input("\033[1;93m[ Shouko.dev ] - Enter cookie: \033[0m").strip()
                        
                        if not cookie:
                            print("\033[1;31m[ Shouko.dev ] - Cookie cannot be empty.\033[0m")
                            input("\033[1;32mPress Enter to continue...\033[0m")
                            continue
                            
                        if package_name:
                            # Login to specific package
                            success = RobloxManager.login_with_cookie(cookie, package_name)
                            if success:
                                print(f"\033[1;32m[ Shouko.dev ] - Successfully logged in to {package_name}\033[0m")
                            else:
                                print(f"\033[1;31m[ Shouko.dev ] - Failed to login to {package_name}\033[0m")
                        else:
                            # Login to all packages
                            packages = RobloxManager.get_roblox_packages()
                            if not packages:
                                print("\033[1;31m[ Shouko.dev ] - No Roblox packages found.\033[0m")
                                input("\033[1;32mPress Enter to continue...\033[0m")
                            ...
                            for pkg in packages:
                                success = RobloxManager.login_with_cookie(cookie, pkg)
                                if success:
                                    print(f"\033[1;32m[ Shouko.dev ] - Successfully logged in to {pkg}\033[0m")
                                else:
                                    print(f"\033[1;31m[ Shouko.dev ] - Failed to login to {pkg}\033[0m")
                                    
                        input("\033[1;32mPress Enter to continue...\033[0m")
                        
                    elif choice == "2":
                        # Generate new cookie
                        username = input("\033[1;93m[ Shouko.dev ] - Enter Roblox username: \033[0m").strip()
                        password = input("\033[1;93m[ Shouko.dev ] - Enter Roblox password: \033[0m").strip()
                        
                        if not username or not password:
                            print("\033[1;31m[ Shouko.dev ] - Username and password cannot be empty.\033[0m")
                            input("\033[1;32mPress Enter to continue...\033[0m")
                            continue
                            
                        cookie = RobloxManager.generate_cookie(username, password)
                        if cookie:
                            print("\033[1;32m[ Shouko.dev ] - Cookie generated successfully!\033[0m")
                            save = input("\033[1;93m[ Shouko.dev ] - Save this cookie? (y/n): \033[0m").strip().lower()
                            if save == 'y':
                                FileManager.add_cookie(cookie)
                        else:
                            print("\033[1;31m[ Shouko.dev ] - Failed to generate cookie.\033[0m")
                            
                        input("\033[1;32mPress Enter to continue...\033[0m")
                        
                    elif choice == "3":
                        # Bypass cookie verification
                        cookie = input("\033[1;93m[ Shouko.dev ] - Enter cookie to bypass: \033[0m").strip()
                        if not cookie:
                            print("\033[1;31m[ Shouko.dev ] - Cookie cannot be empty.\033[0m")
                            input("\033[1;32mPress Enter to continue...\033[0m")
                            continue
                            
                        bypassed_cookie = RobloxManager.bypass_cookie_verification(cookie)
                        if bypassed_cookie:
                            print("\033[1;32m[ Shouko.dev ] - Cookie bypassed successfully!\033[0m")
                            print(f"\033[1;36mBypassed Cookie: {bypassed_cookie}\033[0m")
                            save = input("\033[1;93m[ Shouko.dev ] - Save this bypassed cookie? (y/n): \033[0m").strip().lower()
                            if save == 'y':
                                FileManager.add_cookie(bypassed_cookie)
                        else:
                            print("\033[1;31m[ Shouko.dev ] - Failed to bypass cookie.\033[0m")
                            
                        input("\033[1;32mPress Enter to continue...\033[0m")
                        
                    elif choice == "4":
                        # Mass logout
                        confirm = input("\033[1;93m[ Shouko.dev ] - Are you sure you want to logout from all packages? (y/n): \033[0m").strip().lower()
                        if confirm == 'y':
                            success = RobloxManager.logout_all_packages()
                            if success:
                                print("\033[1;32m[ Shouko.dev ] - Successfully logged out from all packages.\033[0m")
                            else:
                                print("\033[1;31m[ Shouko.dev ] - Failed to logout from some packages.\033[0m")
                        else:
                            print("\033[1;33m[ Shouko.dev ] - Logout cancelled.\033[0m")
                            
                        input("\033[1;32mPress Enter to continue...\033[0m")
                        
                    elif choice == "5":
                        # View saved cookies
                        cookies = FileManager.load_cookies()
                        if not cookies:
                            print("\033[1;31m[ Shouko.dev ] - No cookies found in storage.\033[0m")
                        else:
                            print("\033[1;32m[ Shouko.dev ] - Saved Cookies:\033[0m")
                            for i, cookie in enumerate(cookies, 1):
                                print(f"\033[1;36m{i}. {cookie[:50]}...\033[0m")
                                
                        input("\033[1;32mPress Enter to continue...\033[0m")
                        
                    elif choice == "6":
                        # Add cookie to storage
                        cookie = input("\033[1;93m[ Shouko.dev ] - Enter cookie to save: \033[0m").strip()
                        if not cookie:
                            print("\033[1;31m[ Shouko.dev ] - Cookie cannot be empty.\033[0m")
                            input("\033[1;32mPress Enter to continue...\033[0m")
                            continue
                            
                        success = FileManager.add_cookie(cookie)
                        if success:
                            print("\033[1;32m[ Shouko.dev ] - Cookie saved successfully.\033[0m")
                        else:
                            print("\033[1;31m[ Shouko.dev ] - Failed to save cookie.\033[0m")
                            
                        input("\033[1;32mPress Enter to continue...\033[0m")
                        
                    elif choice == "7":
                        # Clear all cookies
                        confirm = input("\033[1;93m[ Shouko.dev ] - Are you sure you want to clear ALL saved cookies? (y/n): \033[0m").strip().lower()
                        if confirm == 'y':
                            success = FileManager.clear_cookies()
                            if success:
                                print("\033[1;32m[ Shouko.dev ] - All cookies cleared successfully.\033[0m")
                            else:
                                print("\033[1;31m[ Shouko.dev ] - Failed to clear cookies.\033[0m")
                        else:
                            print("\033[1;33m[ Shouko.dev ] - Clear operation cancelled.\033[0m")
                ...
                print(f"\033[1;31m[ Shouko.dev ] - Error in cookie management: {e}\033[0m")
                Utilities.log_error(f"Cookie management error: {e}")
                input("\033[1;32mPress Enter to continue...\033[0m")
